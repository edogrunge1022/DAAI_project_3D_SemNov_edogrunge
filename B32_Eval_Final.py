...
sys.path.insert(0, '/content/gdrive/MyDrive/AI project - 3D semantic novelty detection/SemNov_AML_DAAI_23-24/OpenShape')
...
def G14(s):
    ...
    model_path = os.path.join('/content/gdrive/MyDrive/AI project - 3D semantic novelty detection/SemNov_AML_DAAI_23-24/Pointbert_G14','model.pt')
...
def L14(s):
    ...
    model_path = os.path.join('/content/gdrive/MyDrive/AI project - 3D semantic novelty detection/SemNov_AML_DAAI_23-24/Pointbert_L14','model.pt')
    s = torch.load(model_path)
...
def B32(s):
    ...
    model_path = os.path.join('/content/gdrive/MyDrive/AI project - 3D semantic novelty detection/SemNov_AML_DAAI_23-24/Pointbert_vitB32','model.pt')
    s = torch.load(model_path)
    dic = model.load_state_dict(module(s, 'pc_encoder'))
    print(dic)
    return model
...
def eval_ood_md2sonn(opt, config, s):
    ...
    if opt.src == 'SR1':
        ...
        id_dataset_all = ScanObject(class_choice="sonn_2_mdSet1", **sonn_args) #whole dataset

        id_size = int(0.9 * len(id_dataset_all))
        val_size = len(id_dataset_all) - id_size

        id_dataset_thres, id_dataset = torch.utils.data.random_split(id_dataset_all, [val_size, id_size]) # split the dataset find a fcn to do it

        # id_dataset_thres, id_dataset = SPLIT(id_dataset_all) # split the dataset find a fcn to do it
        id_loader = DataLoader(id_dataset, **dataloader_config) # 90% of the all
        id_loader_thresh = DataLoader(id_dataset_thres, **dataloader_config) # 10% 
        ood1_loader = DataLoader(ScanObject(class_choice="sonn_2_mdSet2", **sonn_args), **dataloader_config)
    ...
    # FEATURES EVALUATION
    eval_OOD_with_feats(model, train_loader, id_loader_thresh, id_loader, ood1_loader, ood2_loader, save_feats=opt.save_feats)
    return
